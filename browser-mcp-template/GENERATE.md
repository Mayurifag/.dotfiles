...
1. Close Chrome for this profile:
   `OPENCODE_BROWSER_PROFILE_DIR=browser-mcp-template/profile browser-mcp --close`
2. If `browser-mcp-template/profile/` exists, ask before deleting it. If approved:
   `rm -rf browser-mcp-template/profile`
3. Launch from chezmoi repo root:
   `OPENCODE_BROWSER_PROFILE_DIR=browser-mcp-template/profile browser-mcp --launch https://www.wikipedia.org/`
4. Verify `loopbackNetworkPermissions.default.label` is `allow`:
   `OPENCODE_BROWSER_PROFILE_DIR=browser-mcp-template/profile browser-mcp --status`
...
14. Delete all smoke userscripts from Tampermonkey trash.
...
3. Install with the CDP command in `Install Userscript`.
...
After opening a `.user.js` URL, click Tampermonkey's hidden install page:

```sh
node browser-mcp-template/cdp-tampermonkey.js install
```

## Delete Smoke Scripts

First try Tampermonkey UI:
...
1. Close target browser: `browser-mcp --close`.
2. Replace target `.opencode/browser-mcp-profile/` with a copy of `~/.local/share/chezmoi/browser-mcp-template/profile/`.
3. Launch from target project: `browser-mcp --launch <url>`.
4. Verify `browser-mcp --status` uses target `.opencode/browser-mcp-profile/`.
...
- Do not debug projects directly with `browser-mcp-template/profile/`; copy it.
